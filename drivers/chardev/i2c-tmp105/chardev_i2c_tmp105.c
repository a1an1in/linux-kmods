...
/*
 * chardev_i2c_tmp105.c - TMP105 的字符设备驱动
 * （学习用：把字符驱动的标准结构完整走一遍）
 *
 * 标准结构清单（代码里用 ①…⑩ 标注对应位置）：
 *   ① 模块参数：总线号 / 从机地址
 ...
 *
 * 与 hwmon_i2c_tmp105 的关系（同一颗芯片，两种暴露方式）：
 *   - hwmon_i2c_tmp105：i2c client 驱动 → hwmon，暴露 temp1_*
 *   - 本模块：字符设备驱动 → /dev/tmp105
 *   本模块刻意**不注册 i2c client**：只用 i2c_get_adapter() 拿
 *   适配器引用 + i2c_transfer 直接发消息，所以不去"占"0x48，
 *   可与 hwmon_i2c_tmp105 同时加载（正好对照两种接口）。
 *
 * 命名/位置见 doc/开发规范.md：drivers/<子系统>/<总线>-<器件>/
 * 本驱动属 chardev/i2c、器件 tmp105，模块名 chardev_i2c_tmp105。
...
#define DRIVER_NAME		"chardev_i2c_tmp105"
#define DEVICE_NAME		"tmp105"	/* → /dev/tmp105 */

/* 器件寄存器指针与换算常数（与 hwmon_i2c_tmp105 同源） */

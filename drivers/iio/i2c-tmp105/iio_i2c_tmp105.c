...
/*
 * iio_i2c_tmp105.c - TI TMP105 / LM75 兼容 I2C 温度传感器驱动（IIO）
 *
 * 命名遵循 doc/开发规范.md：drivers/<子系统>/<总线>-<器件>/
 * 本驱动属 iio/i2c、器件 tmp105，模块名 iio_i2c_tmp105。
 *
 * 与 hwmon_i2c_tmp105 是同一颗芯片的两种外壳（对照学习），
 * 见 doc/iio/i2c-tmp105/设计文档.md §2：
 *   hwmon：驱动算好语义量，sysfs 直接给 m°C（temp1_input）
 *   iio：  驱动只给 raw + scale，用户态按 mC = raw * scale 换算
 *
 * 器件（QEMU hw/sensor/tmp105.c 同款，寄存器表同 hwmon 驱动）：
...
/* ------------------------------------------------------------------ */
/* 寄存器访问：同 hwmon_i2c_tmp105.c 的两条路径 */
/* ------------------------------------------------------------------ */
			...
			const struct i2c_device_id *id)
{
	 ...
	 * （同 hwmon_i2c_tmp105 的 devm 注册，remove 留空）。
	 */
...
/* ------------------------------------------------------------------ */
/* 设备匹配：两套表（同 hwmon_i2c_tmp105 §5.6） */
/* ------------------------------------------------------------------ */

...
#
# iio_tmp105 的端到端验收（宿主侧，覆盖设计文档 §6 的 L1~L4）：
#   1. make 交叉编译（递归构建发现 drivers/i2c/iio_tmp105/Kbuild）
...
#
# 与 hwmon_tmp105/verify_qemu.sh 的差异：
...
#
# 位置：tests/i2c/iio_tmp105/（与 drivers/i2c/iio_tmp105/ 同构）
# 用法：tests/i2c/iio_tmp105/verify_qemu.sh
...
KO_IN_GUEST=/mnt/drivers/i2c/iio_tmp105/iio_tmp105.ko
...
if make -C "$KMODS_ROOT" KDIR="$KDIR" ARCH=arm64 \
	...
	ko=$(ls -l "$KMODS_ROOT/drivers/i2c/iio_tmp105/iio_tmp105.ko" | awk '{print $5}')
	ok "make 成功（iio_tmp105.ko = $ko bytes）"
else
...
echo "== 7. L4：客户机自检脚本 =="
guest_out ri "sh /mnt/tests/i2c/iio_tmp105/read_iio.sh"
...
rm -f "$OUT/loop"
send "n=0; while [ \$n -lt 100 ]; do rmmod iio_tmp105; insmod $KO_IN_GUEST; n=\$((n+1)); done; rmmod iio_tmp105; echo done > /mnt/.verify/loop"

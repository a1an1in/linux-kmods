...
#   8. 共存：与 hwmon_tmp105 同时加载，两个接口读到同一温度
#   9. rmmod 后 /dev/tmp105 消失；100 次 insmod/rmmod 无残留
#  10. insmod 之后的内核日志无 Oops/BUG/WARNING
#
# 位置：tests/i2c/chardev_tmp105/（与 drivers/i2c/chardev_tmp105/ 同构）
# 用法：tests/i2c/chardev_tmp105/verify_qemu.sh
set -u
...
TESTDIR_HOST="$KMODS_ROOT/tests/i2c/chardev_tmp105"
TESTDIR_GUEST=/mnt/tests/i2c/chardev_tmp105
KO_CHAR_GUEST=/mnt/drivers/i2c/chardev_tmp105/chardev_tmp105.ko
KO_HWMON_GUEST=/mnt/drivers/i2c/hwmon_tmp105/hwmon_tmp105.ko
DEV=/dev/tmp105
...
if make -C "$KMODS_ROOT" KDIR="$KDIR" ARCH=arm64 \
	...
	ko=$(ls -l "$KMODS_ROOT/drivers/i2c/chardev_tmp105/chardev_tmp105.ko" |
	     awk '{print $5}')
	ok "make 成功（chardev_tmp105.ko = $ko bytes）"
else
...
if "$CROSS"gcc -static -O2 -Wall \
	-I "$KMODS_ROOT/drivers/i2c/chardev_tmp105" \
...
echo "== 7. 与 hwmon_tmp105 共存（同一颗芯片，两个接口）=="
send "insmod $KO_HWMON_GUEST"
wait_for "hwmon_tmp105 0-0048" 30 && ok "hwmon_tmp105 也加载成功" \
	|| bad "hwmon_tmp105 加载失败（两个模块抢设备？）"
...
[ -n "$(out_value hwkeep)" ] && ok "hwmon_tmp105 不受影响，仍可读：$(out_value hwkeep)" \
	|| bad "hwmon 接口也没了"

echo "== 9. 反复 insmod/rmmod 100 次 =="
rm -f "$OUT/loop"
send "n=0; while [ \$n -lt 100 ]; do rmmod chardev_tmp105; insmod $KO_CHAR_GUEST; n=\$((n+1)); done; rmmod chardev_tmp105; echo done > /mnt/.verify/loop"

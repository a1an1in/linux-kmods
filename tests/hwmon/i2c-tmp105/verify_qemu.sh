...
#
# 位置：tests/hwmon/i2c-tmp105/（与驱动 drivers/hwmon/i2c-tmp105/ 同构，见 doc/开发规范.md）
# 用法：tests/hwmon/i2c-tmp105/verify_qemu.sh
...
KO_IN_GUEST=/mnt/drivers/hwmon/i2c-tmp105/hwmon_i2c_tmp105.ko
...
if make -C "$KMODS_ROOT" KDIR="$KDIR" ARCH=arm64 \
	...
	ko=$(ls -l "$KMODS_ROOT/drivers/hwmon/i2c-tmp105/hwmon_i2c_tmp105.ko" | awk '{print $5}')
	ok "make 成功（hwmon_i2c_tmp105.ko = $ko bytes）"
else
...
guest_out rh "sh /mnt/tests/hwmon/i2c-tmp105/read_hwmon.sh"
...
rm -f "$OUT/loop"
send "n=0; while [ \$n -lt 100 ]; do rmmod hwmon_i2c_tmp105; insmod $KO_IN_GUEST; n=\$((n+1)); done; rmmod hwmon_i2c_tmp105; echo done > /mnt/.verify/loop"
